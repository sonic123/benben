...
- (void)viewDidLoad
{
    ...
    self.barArray=[NSMutableArray arrayWithCapacity:0];
    [self.barTable registerClass:[SLMerchantCell class] forCellReuseIdentifier:MerchantCellIdentifier];
...
-(void)loadData{
    ...
    if (delegate.lat||delegate.lng) {
       [_aiBang searchBizWithCity:@"上海" Query:@"酒吧" Address:@"" Category:@"" Lng:[NSString stringWithFormat:@"%f",delegate.lng] Lat:[NSString stringWithFormat:@"%f",delegate.lat] Radius:@"5000" Rankcode:@"0" From:[NSString stringWithFormat:@"%d",_fromNumber] To:[NSString stringWithFormat:@"%d",_toNumber]];
    }else{
        [_aiBang searchBizWithCity:@"北京" Query:@"酒吧" Address:@"" Category:@"" Lng:@"116.420038" Lat:@"39.908568" Radius:@"5000" Rankcode:@"0" From:[NSString stringWithFormat:@"%d",_fromNumber] To:[NSString stringWithFormat:@"%d",_toNumber]];
    }
    delegate=nil;
}
-(void)showMenu{
...
-(void) requestDidFinishWithData:(NSData*)data aibangApi:(id)aibangApi{
    ...
    if ([SLMerchantsDM parseIntoMerchantstList:self.barArray fromString:data]) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    self.detailView.dataModel=[self.barArray objectAtIndex:indexPath.row];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    SLMerchantCell *cell=[self.barTable dequeueReusableCellWithIdentifier:MerchantCellIdentifier];
    if (!cell) {
        cell=[[SLMerchantCell alloc]initWithStyle:UITableViewCellStyleValue2 reuseIdentifier:MerchantCellIdentifier];
    }
    
    SLMerchantsDM *oneMerchantDM=[self.barArray objectAtIndex:indexPath.row];
